...
object FunSets {
  ...
  def forall(s: Set, p: Int => Boolean): Boolean = {
    val sWithoutP = diff(s,p)
    def iter(a: Int): Boolean = {
      if (a > bound) true
      // contains(s, a) && !p(a)
      // ==
      // contains(s, a) && !contains(p, a)
      // ==
      // contains(diff(s,p), a)
      else if ( contains(sWithoutP, a) ) false
      else iter(a + 1)
    }
    iter(-bound)
  }
  ...
  // foreach; p is not satisfied == not exists
  // ==
  // not ( not exists ) == exists == ! foreach; p is not satisfied
