...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
                       explored: Set[Block]): Stream[(Block, List[Move])] = neighbors.filter{case (block, move) => !explored.contains(block)
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (block, history) #:: xs => {
      ...
      val newNeighbours = newNeighborsOnly(neighborsWithHistory(block, history), alreadyExplored)
      initial.head #:: from(xs ++ newNeighbours, alreadyExplored)
    }
  }
  ...
  lazy val solution: List[Move] = pathsToGoal match {
    ...
    case (block, history) #:: xs => history.reverse
  }
}
