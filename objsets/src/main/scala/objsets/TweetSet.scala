...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    // 1
    left.filter(p) union (right.filter(p) union ( if(p(elem)) acc.incl(elem) else acc ))
  ...
  def mostRetweeted: Tweet = {
    try {
      ...
      if (elem.retweets > mostRetweetedChild.retweets) elem 
      else mostRetweetedChild
    } catch {
...
object GoogleVsApple {
  ...
  def hasKeywords(keywords: List[String])(tweet: Tweet) : Boolean = {
